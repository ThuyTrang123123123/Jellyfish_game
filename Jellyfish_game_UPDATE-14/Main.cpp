...
int main(int argc, char* argv[])
{
    ...
    while (!is_quit)
    {
       ...
       /* if (p_player.GetDonutCount() >= 1)
        {
            p_player.LoadImg("character//adult.png", g_screen);
        }*/
       ...
       // int donut_count = 0;
                ...
                for (int t=0; t<threats_list.size(); t++)
                {
                    ...
                    if (obj_threat != NULL)
                    {
                        ...
                        if (bCol)
                        {
                            ...
                            else if (mark_value >= 20)  speed_multiplier = 2;
        ...
        // INSERT: nếu vừa đạt mốc → cấp thêm 1 mạng
         if (donut_count >= next_life_threshold) {
             player_power.InitCrease();      // +1 mạng, thêm icon tim
             next_life_threshold += 15;
             num_die --;     // nâng mốc lên 30, 45…
         }
        ...
        int time_one_frame = (1000/FRAME_PER_SECOND) / speed_multiplier;
